...
def color_block_std(block: np.ndarray) -> float:
...
def wavelet_smooth(image_array: np.ndarray, wavelet_name: str, level: int, detail_scale: float) -> np.ndarray:
    ...
    for channel_index in range(image_array.shape[2]):
        channel = image_array[:, :, channel_index].astype(np.float32)
        coeffs = pywt.wavedec2(channel, wavelet=wavelet_name, level=level, mode="periodization")
    ...
    if not split_condition(image_array, threshold, min_block_size) or height < 2 or width < 2:
        return [
            QuadNode(
                ...
                std_value=color_block_std(image_array),
            )
        ]
...
def build_segmented_image(shape: Tuple[int, int, int], nodes: List[QuadNode]) -> Tuple[np.ndarray, np.ndarray]:
    segmented = np.zeros(shape, dtype=np.uint8)
    labels = np.zeros(shape[:2], dtype=np.int32)

    for index, node in enumerate(nodes, start=1):
        value = np.clip(np.round(node.mean_value), 0, 255).astype(np.uint8)
...
def compute_mean_region_variance(image_array: np.ndarray, labels: np.ndarray) -> float:
    ...
    for label in np.unique(labels):
        ...
        if region_pixels.size == 0:
            continue
        pixel_count = int(region_pixels.shape[0])
        weighted_variance_sum += float(np.mean(np.var(region_pixels, axis=0))) * pixel_count
...
def difference_heatmap(quadtree_image: np.ndarray, kmeans_image: np.ndarray) -> np.ndarray:
    channel_difference = np.abs(quadtree_image.astype(np.int16) - kmeans_image.astype(np.int16))
    difference = np.mean(channel_difference, axis=2).astype(np.uint8)
...
def create_comparison_figure(results: Dict[str, np.ndarray], metrics: Dict[str, float]):
    ...
    panels = [
        ("Original", results["original_image"], None),
        ("Wavelet Smoothed", results["smoothed_image"], None),
        ("Quadtree Segmented", results["quadtree_segmented"], None),
        ("Quadtree Boundaries", results["quadtree_overlay"], None),
        ("K-Means Segmented", results["kmeans_segmented"], None),
